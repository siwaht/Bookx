...
export function pronunciationRouter(db: SqlJsDatabase): Router {
  ...
  router.get('/', (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to list pronunciation rules' });
    }
  });
  ...
  router.post('/', (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to create pronunciation rule' });
    }
  });
  ...
  router.put('/:ruleId', (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to update pronunciation rule' });
    }
  });
  ...
  router.delete('/:ruleId', (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to delete pronunciation rule' });
    }
  });

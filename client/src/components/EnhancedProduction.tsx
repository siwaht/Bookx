...
import { AudioLines, ChevronDown, Music2, Pause, Play, Plus, SlidersHorizontal, Users, Volume2, Wand2 } from "lucide-react";
...
export function EnhancedProduction({ kind, playing, setPlaying }: { kind: "audiobook" | "podcast"; playing: boolean; setPlaying: (value: boolean) => void }) {
  ...
  const [speakers, setSpeakers] = useState<Speaker>(null as never);
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    ...
    <div className="grid flex-1 gap-5 p-5 xl:grid-cols-[minmax(0,1.2fr)_360px] md:p-8">
      ...
      <aside className="space-y-5">
        <div className="rounded-[22px] border border-[#dfe5de] bg-[#fffefa] p-5"><div className="flex items-center justify-between"><div><span className="mono text-[10px] tracking-[.13em] text-[#678e89]">{podcast ? "MULTI-CAST EPISODE" : "LONG-FORM BATCH"}</span><h3 className="mt-2 font-bold">{podcast ? "People in this episode" : "Generate without losing your place"}</h3></div><Users size={17} className="text-[#4c867f]" /></div>{podcast ? <><div className="mt-4 space-y-2">{cast.map((speaker, index) => <div key={`${speaker.name}-${index}`} className="rounded-xl bg-[#f3f6f1] p-3"><div className="flex items-center gap-3"><span className="grid h-8 w-8 place-items-center rounded-full bg-[#d2e6e0] text-xs font-bold text-[#39716d]">{speaker.name[0]}</span><span className="flex-1 text-xs font-semibold text-[#4b5d60]">{speaker.name} · {speaker.role}</span></div><select value={speaker.voice} onChange={(event) => updateVoice(index, event.target.value)} className="mt-2 w-full rounded-lg border border-[#dce5de] bg-white px-2 py-1.5 text-xs text-[#41776f]"><option>Iris</option><option>Theo</option><option>Sage</option><option>Noor</option></select></div>)}</div><button onClick={addGuest} className="mt-3 w-full rounded-xl border border-dashed border-[#95b8b1] py-2.5 text-xs font-bold text-[#3f7b74]"><Plus className="mr-1 inline" size={13} /> Add another guest</button></> : <><div className="mt-4 rounded-xl bg-[#eef5f1] p-3"><strong className="block text-sm text-[#41746e]">Continue in groups</strong><span className="mt-1 block text-xs leading-5 text-[#718184]">Keep the same cast and mix choices for every batch of chapters.</span></div><div className="mt-3 grid grid-cols-3 gap-2">{["4 chapters", "8 chapters", "Custom"].map((label) => <button key={label} onClick={() => setBatch(label)} className={`rounded-lg px-2 py-2 text-[10px] font-bold ${batch === label ? "bg-[#235f61] text-white" : "bg-[#f1f3ef] text-[#748385]"}`}>{label}</button>)}</div><button onClick={runBatch} disabled={runningBatch} className="mt-3 w-full rounded-xl bg-[#235f61] py-2.5 text-xs font-bold text-white disabled:opacity-60">{runningBatch ? `Preparing ${batch}…` : `Generate ${batch}`}</button></>}</div>
        <div className="rounded-[22px] border border-[#dfe5de] bg-[#fbfbf7] p-5"><span className="mono text-[10px] tracking-[.13em] text-[#678e89]">SOUND SIMPLE</span><label className="mt-4 block text-xs font-bold text-[#526468]">Background music<select value={music} onChange={(event) => setMusic(event.target.value)} className="input mt-2"><option>Opening theme · Gentle rise</option><option>Quiet underscore · Felt piano</option><option>No background music</option></select></label><label className="mt-4 block text-xs font-bold text-[#526468]">Sound effect<select value={effect} onChange={(event) => setEffect(event.target.value)} className="input mt-2"><option>Soft rain at doorway</option><option>Footsteps in hallway</option><option>Cafe room tone</option><option>No sound effect</option></select></label><div className="mt-4"><div className="flex justify-between text-xs font-bold text-[#526468]"><span>Music volume</span><span>{musicVolume}%</span></div><input value={musicVolume} onChange={(event) => setMusicVolume(Number(event.target.value))} type="range" min="0" max="70" className="mt-2 w-full accent-[#3f817a]" /></div><label className="mt-4 flex cursor-pointer items-center justify-between rounded-xl bg-[#edf4f0] p-3"><span className="text-xs font-semibold text-[#4c6566]">Lower music under voices</span><input checked={duck} onChange={(event) => setDuck(event.target.checked)} type="checkbox" className="h-4 w-4 accent-[#3f817a]" /></label></div>
      {notice && <div role="status" className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-full bg-[#203f42] px-4 py-2 text-xs font-semibold text-white shadow-xl">{notice}</div>}
      </aside>
    </div>
  </div>;
}

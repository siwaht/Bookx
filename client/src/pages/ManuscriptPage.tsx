...
const styles: Record<string, React.CSSProperties> = {
  ...
  smallBtn: {
    ...
    background: 'var(--bg-elevated)', color: 'var(--text-secondary)', border: '1px solid var(--border-subtle)', borderRadius: 6, cursor: 'pointer', fontSize: 11, whiteSpace: 'nowrap',
  },
  tinyBtn: {
    display: 'flex', alignItems: 'center', gap: 2, padding: '3px 7px',
    background: 'var(--bg-elevated)', color: 'var(--text-secondary)', border: '1px solid var(--border-subtle)', borderRadius: 5, cursor: 'pointer', fontSize: 10,
  },
  iconBtn: { background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: 2 },
  addChapterForm: { display: 'flex', gap: 4, padding: '8px 12px', borderBottom: '1px solid var(--border-subtle)', alignItems: 'center' },
  inlineInput: {
    flex: 1, padding: '5px 10px', background: 'var(--bg-deep)', color: 'var(--text-primary)', border: '1px solid var(--border-default)',
    borderRadius: 6, fontSize: 12, outline: 'none',
  },
  aiBar: { padding: '8px 12px', borderBottom: '1px solid var(--border-subtle)' },
  aiParseBtn: {
    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, width: '100%',
    padding: '7px 12px', background: 'var(--purple-subtle)', color: 'var(--purple)', border: '1px solid rgba(167,139,250,0.15)',
    borderRadius: 8, cursor: 'pointer', fontSize: 11, fontWeight: 500,
  },
  chapterList: { flex: 1, overflow: 'auto' },
  chapterItem: { position: 'relative', display: 'flex', flexDirection: 'column', borderBottom: '1px solid var(--border-subtle)' },
  chapterBtn: {
    display: 'flex', alignItems: 'center', gap: 4, width: '100%', textAlign: 'left',
    padding: '9px 12px 3px', border: 'none', cursor: 'pointer', fontSize: 12, background: 'transparent',
  },
  chapterNum: { fontSize: 10, color: 'var(--text-muted)', fontFamily: 'monospace', minWidth: 18 },
  chapterTitle: { flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  chapterMeta: { display: 'flex', alignItems: 'center', gap: 4, padding: '2px 12px 7px 30px' },
  progressDot: { width: 8, height: 8, borderRadius: '50%', flexShrink: 0 },
  progressLabel: { fontSize: 10, color: 'var(--text-muted)', flex: 1 },
  menuBtn: { background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: 2 },
  chapterMenu: {
    position: 'absolute', top: '100%', right: 8, zIndex: 20,
    background: 'var(--bg-elevated)', border: '1px solid var(--border-strong)', borderRadius: 10, padding: 4,
    display: 'flex', flexDirection: 'column', gap: 1, minWidth: 140, boxShadow: 'var(--shadow-lg)',
  },
  menuItem: {
    display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px',
    background: 'transparent', color: 'var(--text-secondary)', border: 'none', borderRadius: 6,
    cursor: 'pointer', fontSize: 11, textAlign: 'left', whiteSpace: 'nowrap',
  },
  populateBtn: {
    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, width: '100%',
    padding: '8px 12px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8,
  ...
  textarea: {
    flex: 1, padding: 16, background: 'transparent', color: 'var(--text-primary)', border: 'none',
  ...
  tagPanel: { padding: '8px 14px', borderBottom: '1px solid var(--border-subtle)', background: 'var(--bg-base)', display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 160, overflow: 'auto' },
  tagRow: { display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' },
  tagCatLabel: { fontSize: 10, color: 'var(--text-muted)', minWidth: 60 },
  tagBtn: {
    padding: '3px 8px', background: 'var(--accent-subtle)', color: 'var(--accent)', border: '1px solid rgba(91,141,239,0.15)',
  ...
  wfArrow: { color: 'var(--text-muted)', fontSize: 10 },
  progressBar: { padding: '5px 12px', background: 'var(--bg-base)', borderBottom: '1px solid var(--border-subtle)' },
  statsRow: { display: 'flex', justifyContent: 'space-between', padding: '5px 12px', borderBottom: '1px solid var(--border-subtle)', alignItems: 'center', gap: 6 },
  segmentList: { flex: 1, overflow: 'auto', padding: 5 },
  segmentItem: { padding: 10, borderBottom: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 3, borderRadius: 10, background: 'var(--bg-elevated)' },
  segmentHeader: { display: 'flex', alignItems: 'center', gap: 4 },
  segNum: { fontSize: 9, color: 'var(--text-muted)', fontFamily: 'monospace', minWidth: 22 },
  charSelect: {
    flex: 1, padding: '3px 6px', background: 'var(--bg-deep)', color: 'var(--text-secondary)', border: '1px solid var(--border-default)',
    borderRadius: 6, fontSize: 10, outline: 'none',
  },
  segText: { fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.5, cursor: 'pointer', padding: '3px 6px', borderRadius: 6, border: '1px solid transparent' },
  segEditArea: {
    padding: 10, background: 'var(--bg-deep)', color: 'var(--text-primary)', border: '1px solid var(--border-default)',
    borderRadius: 8, fontSize: 12, lineHeight: 1.5, outline: 'none', resize: 'vertical', fontFamily: 'inherit',
  },
  segStudio: { display: 'flex', flexDirection: 'column', gap: 6, padding: 10, background: 'var(--bg-deep)', borderRadius: 8, border: '1px solid var(--border-subtle)' },
  audioPlayerBox: {
    background: 'var(--success-subtle)', border: '1px solid rgba(74,222,128,0.1)', borderRadius: 10, padding: 10,
  },
  ...
  genBtn: {
    display: 'flex', alignItems: 'center', gap: 4, padding: '7px 16px',
    background: 'var(--success-subtle)', color: 'var(--success)', border: '1px solid rgba(74,222,128,0.15)', borderRadius: 8, cursor: 'pointer', fontSize: 11, fontWeight: 500,
  },
  sendBtn: {
    display: 'flex', alignItems: 'center', gap: 4, padding: '7px 16px',
    border: '1px solid rgba(91,141,239,0.2)', borderRadius: 8, cursor: 'pointer', fontSize: 11, fontWeight: 500,
  },
  genProgress: { display: 'flex', flexDirection: 'column', gap: 3, flex: 1, minWidth: 0 },
  genProgressBar: { height: 4, background: 'var(--bg-elevated)', borderRadius: 4, overflow: 'hidden', width: '100%' },
  genProgressFill: { height: '100%', background: 'var(--accent)', borderRadius: 4, transition: 'width 0.6s cubic-bezier(0.16,1,0.3,1)' },
  genProgressText: { fontSize: 9, color: 'var(--text-tertiary)', whiteSpace: 'nowrap' },
};

...
export default function Home() {
  ...
  const autoCast = () => {
    ...
    recommendCast.mutate({ provider: activeProject.languageModelProvider === "OpenAI" ? "OpenAI" : "Cloudflare", model: activeProject.languageModel || "@cf/openai/gpt-oss-120b", text: manuscript.slice(0, 16_000), voices: voiceLibrary.map((voice) => ({ id: voice.id, label: voice.name, description: voice.detail })) }, {
      onSuccess: (result) => {
        ...
        notify(`${next.length} characters analyzed with ${result.model}. Review or override any voice before generation.`);
      },
  ...
  const notify = (message: string) => { setActivity(message); window.setTimeout(() => setActivity(null), 2600); };
  const handleSecondaryAction = (event: React.MouseEvent<HTMLElement>) => {
    const secondaryFeedback: Record<string, string> = {
      "Split into paragraphs": "Narration paragraphs are ready for review.",
      "Preview": "Preview is ready in the current workspace.",
      "Add delivery tag": "A delivery tag has been added to this local draft.",
      "Suggest direction": "Bookx added a suggested delivery direction.",
      "Background music": "Background music controls are ready in Audio studio.",
      "Sound effects": "Sound-effect controls are ready in Audio studio.",
      "Auto-level voices": "Voice-leveling controls are ready in Audio studio.",
      "Add chapter batch": "A new chapter batch is ready to configure.",
      "Add episode": "A new episode is ready to configure.",
    };
    ...
    if (secondaryFeedback[label]) notify(secondaryFeedback[label]);
    else if (/^Generate (4|8|Custom) chapters$/.test(label)) notify(`${label} is queued with the current cast and mix.`);
    else if (!label && button.parentElement?.textContent?.replace(/\s+/g, " ").includes("CHAPTERS")) notify("A new chapter is ready to configure in this local draft.");
  };
  ...
  const previewCharacter = (character: Character) => {
    ...
    previewCharacterVoice.mutate({ projectId: activeProject.id, characterId: persisted.id, provider: provider as "ElevenLabs" | "OpenAI" | "Deepgram" | "Cloudflare", voiceId: character.voiceId, model: activeProject.voiceModel }, {
      onSuccess: (result) => { setPreviewUrls((current) => ({ ...current, [character.name]: result.audioUrl })); notify(`${character.name} preview is ready.`); },
      onError: () => notify("Bookx could not generate that preview. You can choose another voice or provider."),
    });
  };
  ...
  const previewLibraryVoice = (voice: VoiceChoice) => {
    ...
    previewVoice.mutate({ projectId: activeProject.id, provider: provider as "ElevenLabs" | "OpenAI" | "Deepgram" | "Cloudflare", voiceId: voice.id, model: activeProject.voiceModel, text: `This is a Bookx sample for ${voice.name}. A clear voice helps listeners follow every moment.` }, {
      onSuccess: (result) => { setLibraryPreviewUrls((current) => ({ ...current, [voice.id]: result.audioUrl })); notify(`${voice.name} provider preview is ready.`); },
      onError: () => notify("Bookx could not generate that provider preview. Try another voice or provider."),
    });
  };
  ...
  return (
    <main onClick={handleSecondaryAction} className="min-h-screen bg-[#f7f5ef] text-[#203038] noise-bg">
      {screen === "dashboard" ? (
        <Dashboard projects={projects} isAuthenticated={isAuthenticated} onNew={() => { setDraft(initialDraft); setWizardOpen(true); setWizardStep(1); }} onImport={() => setImportOpen(true)} onOpen={openProject} />
      ) : (
        <Workspace project={activeProject} tab={tab} setTab={setTab} onBack={() => setScreen("dashboard")} onShare={() => notify("Preview link prepared for sharing.")} onSettings={() => setTab("settings")}>
          {tab === "manuscript" && (activeProject.kind === "podcast" ? <PodcastScriptEditor text={manuscriptText} setText={setManuscriptText} onNext={() => setTab("cast")} /> : <Manuscript toolOpen={toolOpen} setToolOpen={setToolOpen} onNext={() => setTab("cast")} />)}
          {tab === "cast" && <CastingReview characters={characters.map((character) => ({ ...character, previewUrl: previewUrls[character.name] }))} voices={filteredVoices} query={voiceSearch} setQuery={setVoiceSearch} prompt={voicePrompt} setPrompt={setVoicePrompt} onFindSimilar={findSimilarVoices} onAutoCast={autoCast} onAddCharacter={addCharacter} isCasting={isCasting} modelLabel={`${activeProject.languageModelProvider || "Cloudflare"} · ${activeProject.languageModel || "@cf/openai/gpt-oss-120b"}`} onPreview={previewCharacter} onPreviewVoice={previewLibraryVoice} onVoiceChange={changeCharacterVoice} libraryPreviewUrls={libraryPreviewUrls} />}
          ...
          {tab === "settings" && <ProviderSettings providers={providers} saved={settingsSaved} checks={providerChecks} onSave={persistProviderDefaults} onValidate={checkProvider} />}
        </Workspace>
      )}
      ...
      {activity && <div role="status" className="fixed right-5 top-20 z-[70] max-w-[calc(100vw-2.5rem)] rounded-2xl bg-[#203f42] px-5 py-3 text-sm font-semibold text-white shadow-xl">{activity}</div>}
    </main>
  );
}

function Dashboard({ projects, isAuthenticated, onNew, onImport, onOpen }: { projects: Project[]; isAuthenticated: boolean; onNew: () => void; onImport: () => void; onOpen: (project: Project) => void }) {
...
function ProjectWizard({ step, setStep, draft, updateDraft, providers, onClose, onCreate, onFile }: { step: number; setStep: (next: number) => void; draft: ProjectSetup; updateDraft: <K extends keyof ProjectSetup>(key: K, value: ProjectSetup[K]) => void; providers: ProviderCatalog[]; onClose: () => void; onCreate: () => void; onFile: (event: ChangeEvent<HTMLInputElement>) => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#173034]/35 p-4 backdrop-blur-sm" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}><div className="soft-shadow flex h-[calc(100dvh-2rem)] max-h-[800px] w-full max-w-3xl flex-col overflow-hidden rounded-[30px] border border-white/70 bg-[#fffefa]"><div className="shrink-0 flex items-start justify-between px-7 pb-5 pt-7"><div><p className="mono text-[10px] tracking-[.16em] text-[#5d918b]">START A NEW PROJECT</p><h2 className="serif mt-2 text-3xl tracking-[-.045em]">{step === 1 ? "Begin with the story." : step === 2 ? "Set the vocal character." : "Review the intent."}</h2></div><IconButton label="Close" onClick={onClose}><X size={17} /></IconButton></div><div className="shrink-0 px-7"><div className="grid grid-cols-3 border-y border-[#e3e7df] py-3">{[[1, "Basics"], [2, "Narration"], [3, "Review"]].map(([number, label]) => <div key={number} className="flex items-center gap-2 text-sm"><span className={`grid h-6 w-6 place-items-center rounded-full text-[11px] font-bold ${step >= Number(number) ? "bg-[#286b69] text-white" : "bg-[#edf0eb] text-[#7a8787]"}`}>{step > Number(number) ? <Check size={13} /> : number}</span><span className={step >= Number(number) ? "font-semibold text-[#2b494b]" : "text-[#859093]"}>{label}</span></div>)}</div></div><div className="min-h-0 flex-1 overflow-y-auto px-7 py-7">
    ...
    {step === 2 && <div className="space-y-6"><div className="rounded-[22px] border border-[#dbe6df] bg-[#f6faf7] p-4"><div className="mb-4 flex items-center justify-between"><div><p className="mono text-[10px] tracking-[.14em] text-[#5b8b84]">MODEL ROUTING</p><p className="mt-1 text-sm text-[#596a6d]">Choose models by task. Bookx keeps the creative workflow independent of any one vendor.</p></div><span className="rounded-full bg-white px-3 py-1 text-[10px] font-bold text-[#4b7d76]">MODEL-AGNOSTIC</span></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Voice provider"><select value={draft.voiceProvider} onChange={(event) => chooseProvider("voiceProvider", "text-to-speech", event.target.value)} className="input">{providers.filter((provider) => provider.capabilities.includes("text-to-speech")).map((provider) => <option key={provider.id} value={provider.id}>{provider.label}{provider.configured ? " · Connected" : ""}</option>)}</select></Field><Field label="Voice model"><select value={draft.voiceModel} onChange={(event) => updateDraft("voiceModel", event.target.value)} className="input">{voiceModels.map((model) => <option key={model.id} value={model.id}>{model.label}{model.detail ? ` · ${model.detail}` : ""}</option>)}</select></Field><Field label="Language-model provider"><select value={draft.languageModelProvider} onChange={(event) => chooseProvider("languageModelProvider", "language-model", event.target.value)} className="input">{providers.filter((provider) => provider.capabilities.includes("language-model")).map((provider) => <option key={provider.id} value={provider.id}>{provider.label}{provider.configured ? " · Connected" : ""}</option>)}</select></Field><Field label="Language model"><select value={draft.languageModel} onChange={(event) => updateDraft("languageModel", event.target.value)} className="input">{languageModels.map((model) => <option key={model.id} value={model.id}>{model.label}{model.detail ? ` · ${model.detail}` : ""}</option>)}</select></Field><Field label="Language"><select value={draft.language} onChange={(event) => updateDraft("language", event.target.value)} className="input"><option>Auto-detect</option><option>English</option><option>Spanish</option><option>French</option><option>German</option></select></Field></div></div><label className="group flex cursor-pointer flex-col items-center justify-center rounded-[22px] border border-dashed border-[#87ada6] bg-[#f1f8f6] px-6 py-8 text-center"><Upload size={22} className="mb-3 text-[#3b7b75]" /><strong>{draft.manuscriptName || "Upload your manuscript"}</strong><span className="mt-2 text-sm text-[#718083]">EPUB, DOCX, TXT, Markdown, or HTML</span><input type="file" accept=".epub,.docx,.txt,.md,.html,.htm" onChange={onFile} className="hidden" /></label><p className="rounded-xl bg-[#fbf4e5] px-4 py-3 text-sm leading-6 text-[#8a6b37]"><Sparkles className="mr-2 inline" size={15} />Bookx will prepare chapters and narration paragraphs for your review. You can also add a manuscript later.</p></div>}
  ...
  </div><div className="flex items-center justify-between border-t border-[#e4e7e0] bg-[#fafaf5] px-7 py-5"><button onClick={() => step === 1 ? onClose() : setStep(step - 1)} className="inline-flex items-center gap-2 text-sm font-semibold text-[#667477]"><ArrowLeft size={15} /> {step === 1 ? "Cancel" : "Back"}</button><button disabled={!canContinue} onClick={() => step < 3 ? setStep(step + 1) : onCreate()} className="inline-flex items-center gap-2 rounded-xl bg-[#235f61] px-5 py-3 text-sm font-bold text-white disabled:opacity-40">{step < 3 ? "Continue" : `Create ${draft.kind === "audiobook" ? "audiobook" : "podcast"}`} <ArrowRight size={16} /></button></div></div></div>;
}

function ImportAudio({ onClose, onFinish }: { onClose: () => void; onFinish: () => void }) { return <div className="fixed inset-0 z-50 grid place-items-center bg-[#173034]/35 p-4 backdrop-blur-sm"><div className="soft-shadow w-full max-w-lg rounded-[28px] bg-[#fffefa] p-7"><div className="flex items-start justify-between"><div><p className="mono text-[10px] tracking-[.15em] text-[#5d918b]">IMPORT AUDIO</p><h2 className="serif mt-2 text-3xl">Bring in your mix.</h2></div><IconButton label="Close" onClick={onClose}><X size={17} /></IconButton></div><label className="mt-7 flex cursor-pointer flex-col items-center rounded-[22px] border border-dashed border-[#90ada7] bg-[#f2f8f5] px-5 py-10 text-center"><FileAudio size={25} className="mb-3 text-[#3c7b75]" /><strong>Choose audio files</strong><span className="mt-2 text-sm text-[#728084]">MP3, WAV, M4A, FLAC, or AAC</span><input type="file" multiple className="hidden" /></label><p className="mt-4 text-sm leading-6 text-[#788689]">Upload one file for a single episode, or several files to create a chaptered project.</p><button onClick={onFinish} className="mt-7 w-full rounded-xl bg-[#235f61] py-3 text-sm font-bold text-white">Continue to project setup</button></div></div>; }
...
function Manuscript({ toolOpen, setToolOpen, onNext }: { toolOpen: boolean; setToolOpen: (value: boolean) => void; onNext: () => void }) {
...
function MultiCastStudio({ kind, playing, setPlaying }: { kind: "audiobook" | "podcast"; playing: boolean; setPlaying: (value: boolean) => void }) {
  ...
  const [speakers, setSpeakers] = useState(kind === "podcast" ? ["Nadia · Host", "Mika · Guest", "Otto · Guest"] : ["Mara · Narrator", "Elias · Character", "June · Character"]);
  const [batch, setBatch] = useState("4 chapters");
  const addSpeaker = () => setSpeakers((current) => [...current, `Guest ${current.length + 1} · Guest`]);
  const isPodcast = kind === "podcast";
  return <div className="flex min-h-0 flex-1 flex-col"><PageHeader eyebrow={isPodcast ? "PODCAST PRODUCTION" : "LONG-FORM PRODUCTION"} title={isPodcast ? "Build every episode around the voices." : "Keep a long story beautifully paced."} description={isPodcast ? "Set a host and guests, give each person a voice, then add music and sound moments without a complicated console." : "Batch chapters, protect narrator consistency, and shape a subtle soundtrack for a long listening session."} actions={<button className="rounded-xl bg-[#235f61] px-3 py-2 text-xs font-bold text-white"><Plus className="mr-1 inline" size={14} /> {isPodcast ? "Add episode" : "Add chapter batch"}</button>} /><div className="grid flex-1 gap-5 p-5 xl:grid-cols-[minmax(0,1.2fr)_360px] md:p-8"><section className="space-y-5"><div className="panel-shadow rounded-[22px] border border-[#dfe5de] bg-[#fffefa] p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-4"><button onClick={() => setPlaying(!playing)} className="grid h-12 w-12 place-items-center rounded-full bg-[#235f61] text-white">{playing ? <Pause size={18} /> : <Play className="ml-0.5" size={18} />}</button><div><strong className="block">{isPodcast ? "Episode 04 · The shared horizon" : "Chapter 01 · Undertow"}</strong><span className="mt-1 block text-xs text-[#7a8789]">{playing ? "Playing mix · 01:24 / 03:18" : "Ready for a quick listen · 03:18"}</span></div></div><span className="rounded-full bg-[#e8f3ef] px-3 py-1.5 text-xs font-bold text-[#417a71]">One simple mix</span></div></div><div className="overflow-hidden rounded-[22px] border border-[#dfe5de] bg-[#fbfcf9]"><div className="flex h-10 items-center border-b border-[#e6eae4] px-5 mono text-[10px] tracking-[.14em] text-[#819091]"><span className="w-[170px]">MIX TRACKS</span><div className="flex flex-1 justify-between"><span>00:00</span><span>01:00</span><span>02:00</span><span>03:00</span></div></div>{[[isPodcast ? "Host + guests" : "Narration", "#70a49b", [25, 48, 74]], ["Sound effect · Rain", "#8a9fbc", [10, 67]], ["Music bed · Opening theme", "#d4a260", [0, 58]]].map(([label, color, clips]) => <div key={label as string} className="flex min-h-16 items-center border-b border-[#e9ece7] px-5 last:border-0"><div className="w-[170px] text-xs font-semibold text-[#4e6063]">{label as string}</div><div className="relative h-9 flex-1 rounded-lg bg-[#eef1ec]">{(clips as number[]).map((left, index) => <div key={index} className="absolute top-1.5 h-6 rounded-md opacity-90" style={{ left: `${left}%`, width: `${index === 1 ? 20 : 18}%`, background: color as string }} />)}</div></div>)}</div><div className="grid gap-3 pt-5 md:grid-cols-3"><button className="rounded-2xl border border-[#dfe5de] bg-[#fffefa] p-4 text-left hover:border-[#99c2b8]"><Music2 size={16} className="mb-3 text-[#4d887f]" /><strong className="block text-sm">Background music</strong><span className="mt-1 block text-xs text-[#7b888a]">Choose, loop, and duck below voices.</span></button><button className="rounded-2xl border border-[#dfe5de] bg-[#fffefa] p-4 text-left hover:border-[#99c2b8]"><AudioLines size={16} className="mb-3 text-[#4d887f]" /><strong className="block text-sm">Sound effects</strong><span className="mt-1 block text-xs text-[#7b888a]">Place moments without cluttering the story.</span></button><button className="rounded-2xl border border-[#dfe5de] bg-[#fffefa] p-4 text-left hover:border-[#99c2b8]"><SlidersHorizontal size={16} className="mb-3 text-[#4d887f]" /><strong className="block text-sm">Auto-level voices</strong><span className="mt-1 block text-xs text-[#7b888a]">Keep every speaker clear and comfortable.</span></button></div></section><aside className="space-y-5"><div className="rounded-[22px] border border-[#dfe5de] bg-[#fffefa] p-5"><div className="flex items-center justify-between"><div><span className="mono text-[10px] tracking-[.13em] text-[#678e89]">{isPodcast ? "MULTI-CAST EPISODE" : "LONG-FORM BATCH"}</span><h3 className="mt-2 font-bold">{isPodcast ? "People in this episode" : "Generate without losing your place"}</h3></div><Users size={17} className="text-[#4c867f]" /></div>{isPodcast ? <><div className="mt-4 space-y-2">{speakers.map((speaker, index) => <div key={`${speaker}-${index}`} className="flex items-center gap-3 rounded-xl bg-[#f3f6f1] p-3"><span className="grid h-8 w-8 place-items-center rounded-full bg-[#d2e6e0] text-xs font-bold text-[#39716d]">{speaker[0]}</span><span className="flex-1 text-xs font-semibold text-[#4b5d60]">{speaker}</span><span className="rounded-lg bg-white px-2 py-1 text-[10px] text-[#497c77]">Voice assigned</span></div>)}</div><button onClick={addSpeaker} className="mt-3 w-full rounded-xl border border-dashed border-[#95b8b1] py-2.5 text-xs font-bold text-[#3f7b74]"><Plus className="mr-1 inline" size={13} /> Add another guest</button></> : <><div className="mt-4 rounded-xl bg-[#eef5f1] p-3"><strong className="block text-sm text-[#41746e]">Continue in groups</strong><span className="mt-1 block text-xs leading-5 text-[#718184]">Bookx keeps the same cast, pronunciation rules, and mix settings as you generate a book in batches.</span></div><div className="mt-3 grid grid-cols-3 gap-2">{["4 chapters", "8 chapters", "Custom"].map((label) => <button key={label} onClick={() => setBatch(label)} className={`rounded-lg px-2 py-2 text-[10px] font-bold ${batch === label ? "bg-[#235f61] text-white" : "bg-[#f1f3ef] text-[#748385]"}`}>{label}</button>)}</div><button className="mt-3 w-full rounded-xl bg-[#235f61] py-2.5 text-xs font-bold text-white">Generate {batch}</button></>}</div><div className="rounded-[22px] border border-[#dfe5de] bg-[#fbfbf7] p-5"><span className="mono text-[10px] tracking-[.13em] text-[#678e89]">SOUND SIMPLE</span><div className="mt-4"><label className="text-xs font-bold text-[#526468]">Background music</label><select className="input mt-2"><option>Opening theme · Gentle rise</option><option>Quiet underscore · Felt piano</option><option>No background music</option></select></div><div className="mt-4"><label className="text-xs font-bold text-[#526468]">Sound effect</label><select value={effect} onChange={(event) => setEffect(event.target.value)} className="input mt-2"><option>Soft rain at doorway</option><option>Footsteps in hallway</option><option>Cafe room tone</option><option>No sound effect</option></select></div><div className="mt-4"><div className="flex justify-between text-xs font-bold text-[#526468]"><span>Music volume</span><span>{musicVolume}%</span></div><input value={musicVolume} onChange={(event) => setMusicVolume(Number(event.target.value))} type="range" min="0" max="70" className="mt-2 w-full accent-[#3f817a]" /></div><label className="mt-4 flex cursor-pointer items-center justify-between rounded-xl bg-[#edf4f0] p-3"><span className="text-xs font-semibold text-[#4c6566]">Lower music under voices</span><input checked={duck} onChange={(event) => setDuck(event.target.checked)} type="checkbox" className="h-4 w-4 accent-[#3f817a]" /></label></div></aside></div></div>; }
...
function Export({ readiness, exported, onExport }: { readiness: ReturnType<typeof projectReadiness>; exported: boolean; onExport: () => void }) { const [format, setFormat] = useState("ACX"); const packageLabel = format === "InAudio package" ? format : `${format} package`; return <div className="flex min-h-0 flex-1 flex-col"><PageHeader eyebrow="06 · PUBLISH" title="Publish & export." description="Package audio, metadata, and chapter structure for the way your audience listens." /><div className="grid flex-1 gap-6 p-6 lg:grid-cols-[minmax(0,1fr)_330px] md:p-9"><section><div className="grid gap-3 md:grid-cols-3">{[["ACX", "Audiobook delivery", BookOpen], ["Podcast", "Episode package", Mic2], ["InAudio package", "Chaptered distribution", Headphones]].map(([name, detail, Icon]) => <button key={name as string} onClick={() => setFormat(name as string)} className={`rounded-[20px] border p-5 text-left ${format === name ? "border-[#8eb8ae] bg-[#edf7f3]" : "border-[#dfe5de] bg-[#fffefa]"}`}>{createElement(Icon as typeof BookOpen, { size: 18, className: "mb-5 text-[#4d887f]" })}<strong className="block text-sm">{name as string}</strong><span className="mt-1 block text-xs text-[#788688]">{detail as string}</span></button>)}</div><div className="mt-5 rounded-[22px] border border-[#dfe5de] bg-[#fffefa] p-5"><span className="mono text-[10px] tracking-[.13em] text-[#7e8d8e]">{packageLabel.toUpperCase()}</span><h3 className="serif mt-2 text-2xl">Ready when the last line is.</h3><div className="mt-5 space-y-3 text-sm text-[#5d6f71]">{["Chapter audio in a consistent final mix", "Delivery-ready file naming and metadata", "Cover art and chapter sequence", "Validation report included with the package"].map((line) => <div key={line} className="flex items-center gap-3"><CheckCircle2 size={16} className="text-[#5c9688]" />{line}</div>)}</div></div></section><aside className="rounded-[24px] border border-[#dfe5de] bg-[#fbfbf7] p-6"><span className="mono text-[10px] tracking-[.13em] text-[#688f8a]">DELIVERY STATUS</span><div className="mt-4 flex items-center gap-3"><span className={`grid h-10 w-10 place-items-center rounded-full ${readiness.readyToExport ? "bg-[#e2f1eb] text-[#3d806f]" : "bg-[#fff0da] text-[#ae7b2d]"}`}>{readiness.readyToExport ? <Check size={19} /> : <AlertCircle size={19} />}</span><div><strong className="block text-sm">{readiness.readyToExport ? "Ready to package" : "One item remains"}</strong><span className="block text-xs text-[#7c898a]">{readiness.completed} of {readiness.total} pre-flight checks</span></div></div><button onClick={onExport} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-[#235f61] py-3 text-sm font-bold text-white"><Download size={15} /> Create {packageLabel}</button>{exported && <div className="mt-4 rounded-xl bg-[#e7f4ef] p-3 text-xs font-semibold text-[#3f7e70]"><CheckCircle2 className="mr-1 inline" size={14} /> Package prepared. Download is ready in your library.</div>}<p className="mt-5 text-xs leading-5 text-[#7c898a]">The exact delivery label is preserved for every export: ACX, Podcast, and InAudio package.</p></aside></div></div>; }

function Settings({ saved, onSave }: { saved: boolean; onSave: () => void }) { return <div className="flex min-h-0 flex-1 flex-col"><PageHeader eyebrow="SYSTEM" title="Settings" description="Configure providers, defaults, and the way new projects begin." /><div className="grid flex-1 gap-6 p-6 lg:grid-cols-[minmax(0,1fr)_330px] md:p-9"><section className="space-y-5"><div className="rounded-[22px] border border-[#dfe5de] bg-[#fffefa] p-5"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e7f2ef] text-[#3f7e74]"><KeyRound size={18} /></span><div><h3 className="font-bold">TTS providers</h3><p className="text-sm text-[#788689]">Add keys server-side before generating real audio.</p></div></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="ElevenLabs API key"><input type="password" placeholder="Add ElevenLabs key" className="input" /></Field><Field label="OpenAI API key"><input type="password" placeholder="Add OpenAI key" className="input" /></Field></div><p className="mt-4 rounded-xl bg-[#f4f6f1] p-3 text-xs leading-5 text-[#758385]"><AlertCircle className="mr-1 inline text-[#a87934]" size={13} />Provider keys are not stored in the browser. Configure production secrets before enabling real TTS generation.</p></div><div className="rounded-[22px] border border-[#dfe5de] bg-[#fffefa] p-5"><h3 className="font-bold">Project defaults</h3><div className="mt-5 grid gap-4 sm:grid-cols-3"><Field label="Default voice model"><select className="input"><option>Multilingual v2</option><option>Eleven v3</option><option>Flash draft</option></select></Field><Field label="Narration pace"><select className="input"><option>Natural</option><option>Measured</option><option>Brisk</option></select></Field><Field label="Chapter gap"><select className="input"><option>2 seconds</option><option>1 second</option><option>3 seconds</option></select></Field></div></div></section><aside className="rounded-[22px] bg-[#294f50] p-6 text-white"><Globe2 size={20} className="text-[#c8dfda]" /><h3 className="serif mt-5 text-3xl">A voice you can keep.</h3><p className="mt-4 text-sm leading-6 text-[#d0e1dc]">Your default choices carry through each project, while every chapter can still be refined on its own.</p><button onClick={onSave} className="mt-6 w-full rounded-xl bg-white py-3 text-sm font-bold text-[#285d60]">{saved ? <><Check className="mr-1 inline" size={14} /> Saved</> : "Save settings"}</button></aside></div></div>; }
